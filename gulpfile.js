...
function writeComponents(meta) {
  ...
  const jacareJs = `/* Auto-generated by gulp-icon-font — Jacaré helpers
...
`;
  ...
  const jacareJcr = `import { swIconClass } from './jacare.js'
...
export <view>
  <i
    class=\${() => swIconClass(read(name))}
    style---size=\${() => \`\${read(size)}px\`}
    style---color=\${() => read(color)}
    role="img"
    aria-label=\${() => read(label) || read(name)}
  ></i>
</view>
...
`;
  ...
  fs.writeFileSync(path.join(outDir, "jacare.js"), jacareJs);
  fs.writeFileSync(path.join(outDir, "SwIcon.jcr"), jacareJcr);
  fs.writeFileSync(
...
Also load \`../iconfont.css\` (or the SVG sprite \`../${icons.name}.sprite.svg\`).
`
  );
}
